...
function get_tempinfo()
{
	...
	// From https://github.com/fabianonline/cacti-scripts/blob/master/hddtemp.php
	if (is_file('/etc/default/hddtemp')) {
		...
		if (!$fp) {
			...
			foreach($disks as $id=>$d) {
				$data = explode("|", $d);
				$diskTemp[$data[0]] = array(
					'device' => $data[1],
					'temp' => $data[2] . $data[3]
				);
			}

			// only show first disk /dev/sda
			$keys = array_keys($diskTemp);
			$info['disk'] = floatVal($diskTemp[$keys[0]]['temp']);
		}
	}
...
<?php if (isset($tempinfo['cpu'])) : ?>
	...
	<td><?php __('Disk Temperature'); ?></td>
	<td><span id="disk_temp"><?php echo $tempinfo['disk'];?></span></td>
	</tr>
<?php endif; ?>
...
function getSysinfo() {
	$.getJSON('?method=sysinfo', function (data) {
		...
		if (data.tempinfo.disk)
			$('#disk_temp').html(data.tempinfo.disk+' ℃')
